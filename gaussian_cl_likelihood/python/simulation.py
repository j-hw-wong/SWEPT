...
from python import like_cl_wishart as like_w
...
def single_obs_cls(n_zbin, pos_pos_in_dir, she_she_in_dir, pos_she_in_dir, lmax, lmin_in, pos_nl_path, she_nl_path,
                   nside, lmin_out, save_dir, mask_path=None, verbose=True):
...
#                   nside, lmin_out, mask_path=None, verbose=True):
    ...
    Args:
        ...
        #JW: save_dir - directory to save Cls
    Returns:
...
def sim_bps(n_zbin, pos_pos_in_dir, she_she_in_dir, pos_she_in_dir, lmax, lmin_in, pos_nl_path, she_nl_path,
            nside, n_bandpower, lmin_out, mask_path, n_loop, batch_size, save_dir, bandpower_spacing='log',
    ...
    if bandpower_spacing == 'log':
        pbl = get_binning_matrix(n_bandpower, lmin_out, lmax, bp_spacing='log')
    elif bandpower_spacing == 'lin':
        pbl = get_binning_matrix(n_bandpower, lmin_out, lmax, bp_spacing='lin')
    elif bandpower_spacing == 'custom':
        pbl = get_binning_matrix(n_bandpower, lmin_out, lmax, bp_spacing='custom', bp_edges=bandpower_edges)
...
def noise_cls(gals_per_sq_arcmin, sigma_e, lmin, lmax, save_dir):
    ...
    ell = np.arange(lmin, lmax + 1)
    pos_nl = 1*(1 / n_i) * np.ones_like(ell)
    #pos_nl = np.zeros(len(ell))
    she_nl = 1*(sigma_e ** 2 / n_i) * np.ones_like(ell)
    #she_nl = np.zeros(len(ell))
...
def noise_cls_3x2pt(n_zbins, sigma_e, lmin, lmax, save_dir):
    """
    JW ammended version of simulation.noise_cls() [Save noise power spectra to file.]
    For more complex tomographic configurations of 3x2pt cosmology we need different levels of noise
    for each redshift bin (caused e.g. by different number densities of galaxies in each bin.) simulation.noise_cls
    assumes bins equally spaced in redshift and with ~equal n(z) per bin, using a simple Gaussian to approximate
    the distribution. For a more generalised observation this is not the case.
    Here, we assume the the Poisson Cls are pre-calculated elsewhere (e.g. see JW YEET measure_3x2pt_cls.py from Hpx
    maps) but the shear Cls and galaxy-galaxy lensing Cls
