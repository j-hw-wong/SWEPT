...
for j in bins:
    for i in bins:
        if i >= j:
            ...
            ell = open_dat(save_dir + 'measured_3x2pt_bps/shear_bp/Cl_EE/ell.txt')
...
for i in bins:
    for j in bins:
        if i >= j:
            ...
            ax.set_ylim(-1.1*abs(min(ymins[j-1])), 1.1*max(ymaxs[j-1]))
...
plt.savefig(save_dir + 'shear_cl.png')
plt.show()
...
for i in bins:
    for j in bins:
        if i >= j:
            ...
            plt.errorbar(ell, ((bp_measured) / (bp)) - 1, xerr=None, yerr=yerr, label='Fractional\nDifference',
            ...
            if i == 1 and j == 1:
                ax.legend(bbox_to_anchor=(0.55, 1.7), fontsize=13.5)

            if j == 1:
                plt.xlabel("$\\ell$", fontsize=15)

            if i == j:
                labelstr = str("$\\frac{C_b^{\gamma\gamma, \\mathrm{Measured}}}{C_b^{\gamma\gamma, \\mathrm{Theory}}}-1$")
                plt.ylabel(labelstr, fontsize=15)
                plt.ticklabel_format(style='sci', axis='y', scilimits=(0, 0))
...
plt.savefig(save_dir + 'shear_cl_residuals.png')
plt.show()
