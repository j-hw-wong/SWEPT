...
def setup_old(obs_bp_path, binmixmat_path, mixmats, mix_lmin, cov_path, pos_nl_path, she_nl_path, noise_lmin,
          input_lmax, n_zbin):
    """
...
def setup(obs_bp_path, mixmats, mix_lmin, cov_path, pos_nl_path, she_nl_path, noise_lmin, input_lmin,
          input_lmax, n_zbin):
    """
    Load and precompute everything that is fixed throughout parameter space. This should be called once per analysis,
    prior to any calls to execute.
...
def execute(theory_cl, theory_lmin, config, noise_cls, pbl_nn, pbl_ne, pbl_ee):
    """
    ...
    for spec_idx, spec in enumerate(spectra):
        ...
        if spec == 'NN':
            ...
            print('pbl shape')
            print(pbl_nn.shape, mixmat_nn_to_nn.shape, this_cl.shape, this_noise_cl.shape)
...
def exp_bp(theory_cl, theory_lmin, config, noise_cls, pbl_nn, pbl_ne, pbl_ee):
    """
    Calculate the joint log-likelihood at a particular point in parameter space.
...
def execute_old(theory_cl, theory_lmin, config, noise_cls, pbl, hpx_win=False, nside=64):
    """
    Calculate the joint log-likelihood at a particular point in parameter space.
...
def exp_bp_old(theory_cl, theory_lmin, config, noise_cls, pbl, hpx_win=False, nside=64):
    """
    Calculate the joint log-likelihood at a particular point in parameter space.
